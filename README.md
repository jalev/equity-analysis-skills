...
If contributing from a submodule, work directly inside the submodule working tree:

```bash
cd .claude/commands          # inside your research repo
git checkout -b improve-bear-case
```

If contributing standalone:
...
Modify the relevant `.md` file. Keep changes focused — one skill per PR is strongly preferred.

**3. Run a benchmark**

Copy `_benchmarks/template.md` to `_benchmarks/_contributions/YYYY-MM-DD-<skill>/author.md` and complete it. The benchmark must:

- Use a real filing (10-K or 10-Q) against the modified skill
- Run the skill to completion and capture the output grade and evidence
- Identify what specifically improved vs the prior version
- Be reproducible: the reviewer must be able to independently replicate your setup

```bash
cp _benchmarks/template.md _benchmarks/_contributions/2026-04-01-bear-case/author.md
# complete the benchmark, then:
git add _benchmarks/_contributions/
```

**4. Add a SKILLS_CHANGELOG entry**

Prepend an entry to `SKILLS_CHANGELOG.md` following the format in that file. Leave the reviewer fields blank — the reviewer fills those in.

**5. Open the PR**

Push your branch and open a PR against `main`. The PR description should link to your benchmark file and summarise what changed and why.

```bash
git add bear-case.md _benchmarks/_contributions/2026-04-01-bear-case/author.md SKILLS_CHANGELOG.md
git commit -m "improve bear case: require combined downside scenario"
git push origin improve-bear-case
# open PR on GitHub
```

If you're working from a submodule, push from inside the submodule working tree. Your research repo's private data never touches this repo.

**6. Reviewer validation**

A maintainer will independently run the modified skill against their own filing data — without seeing your output first — and file `_benchmarks/_contributions/YYYY-MM-DD-<skill>/reviewer.md`. The PR is approved only after the reviewer benchmark is committed.

### What counts as a benchmark

A valid benchmark must show:

- Which filing was used (company, period, form type)
- Which skill was run and with what instruction version
- Output quality assessment against the analytical standard in `CLAUDE.md`
- Grade for the relevant memo(s), with specific evidence
- Comparison to the prior skill version (what improved, what did not)

See `_benchmarks/template.md` for the required structure and `_benchmarks/CLAUDE.md` / `_benchmarks/CODEX.md` for worked examples.

### Reviewer independence requirement

The reviewer must run the skill independently — no access to the author's output before completing their own run. This prevents anchoring. The reviewer files their benchmark as a separate file in the same `_contributions/` subdirectory.

If the reviewer's results materially contradict the author's, the discrepancy must be explained in the PR before merging.

### Contributing a benchmark (without changing a skill)

Benchmarks without a corresponding skill change are also welcome. File them under `_benchmarks/` with a descriptive name (e.g. `_benchmarks/claude-opus-4-nvda-2026.md`) and open a PR. No `SKILLS_CHANGELOG` entry required — add a note to the benchmark itself explaining what it covers.
